...
class DatasetCatalog(object):
    ...
    DATASETS = {
        ...
        "cityscapes_fine_instanceonly_seg_train_cocostyle": {
            "img_dir": "org/train",
            "ann_file": "org/train.coco.json"
        },
        "cityscapes_fine_instanceonly_seg_val_cocostyle": {
            "img_dir": "org/val",
            "ann_file": "org/val.coco.json"
        },
        ...
        "foggy_cityscapes_fine_instanceonly_seg_train_cocostyle": {
            "img_dir": "fog/train",
            "ann_file": "fog/train.coco.json"
        },
        "foggy_cityscapes_fine_instanceonly_seg_val_cocostyle": {
            "img_dir": "fog/val",
            "ann_file": "fog/val.coco.json"
        },
        'sim10k_cocostyle': {
    ...
    @staticmethod
    def get(name, is_custom=False):
        if "coco" in name:
            data_dir = DatasetCatalog.DATA_DIR
            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                root=os.path.join(data_dir, attrs["img_dir"]) if not is_custom else attrs["img_dir"],
                ann_file=os.path.join(data_dir, attrs["ann_file"]) if not is_custom else os.path.join("custom_cache", attrs["ann_file"]),
            )
            return dict(
                factory="COCODataset",
                args=args,
    ...
    @staticmethod
    def insert(name, img_dir, ann_file):
        # name should contain coco or voc
        DatasetCatalog.DATASETS[name] = {
            "img_dir": img_dir,
            "ann_file": ann_file
        }
